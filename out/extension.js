...
function activate(context) {
     ...
     */
    context.subscriptions.push(vscode.commands.registerCommand('my-extension.copyToNotepad', async (item) => {
        try {
            /**
    ...
    context.subscriptions.push(vscode.commands.registerCommand('my-extension.copyAllToNotepad', async () => {
        try {
            const treeView = vscode.window.createTreeView('filteredFiles', {
                treeDataProvider: filteredFilesProvider,
                showCollapseAll: false
            ...
            const relativePaths = selectedItems.map((item) => vscode.workspace.asRelativePath(item.resourceUri));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('my-extension.clearFileFilter', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('my-extension.quickFileFilter', async () => {
        // Show input box
...
class FilteredFilesProvider {
    ...
    // Set filter pattern and update results
    async setFilterPattern(filterPattern) {
        await this.updateFilteredFiles(filterPattern);
        this.refresh();
    }
    // Update filtered files list
    async updateFilteredFiles(filterPattern) {
        if (!filterPattern) {
            this.files = [];
            return;
        }
        try {
            ...
            const filteredFiles = allFiles
                ...
                const fileName = path.basename(file.fsPath);
                const relativePath = vscode.workspace.asRelativePath(file.fsPath);
                return new FileItem(fileName, relativePath, file, vscode.TreeItemCollapsibleState.None);
            });
...
class FileItem extends vscode.TreeItem {
    constructor(label, description, resourceUri, collapsibleState) {
